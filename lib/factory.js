...
function verifyState( state, FLG ) {
	...
	if ( state[ SEED_SECTION_OFFSET ] !== state.length-STATE_FIXED_LENGTH ) {
		return new RangeError( format( '0oh7J', s1, state.length-STATE_FIXED_LENGTH, state[ SEED_SECTION_OFFSET ] ) );
	}
	return null;
}
...
function factory( options ) {
	...
	if ( arguments.length ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0oh2V', options ) );
		}
		if ( hasOwnProp( options, 'copy' ) ) {
			opts.copy = options.copy;
			if ( !isBoolean( options.copy ) ) {
				throw new TypeError( format( '0oh2o', 'copy', options.copy ) );
			}
		}
		if ( hasOwnProp( options, 'state' ) ) {
			...
			if ( !isInt32Array( state ) ) {
				throw new TypeError( format( '0oh7K', 'state', state ) );
			}
		...
		if ( seed === void 0 ) {
			if ( hasOwnProp( options, 'seed' ) ) {
				...
				if ( isPositiveInteger( seed ) ) {
					if ( seed > MAX_SEED ) {
						throw new RangeError( format( '0oh7L', 'seed', seed ) );
					}
					...
					throw new TypeError( format( '0oh7M', 'seed', seed ) );
				}
			} else {
	...
	*/
	function setState( s ) {
		var err;
		if ( !isInt32Array( s ) ) {
			throw new TypeError( format( '0oh7N', s ) );
		}
